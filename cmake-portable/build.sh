TERMUX_PKG_HOMEPAGE=https://cmake.org/
TERMUX_PKG_DESCRIPTION="Family of tools designed to build, test and package software"
TERMUX_PKG_LICENSE="BSD 3-Clause"
TERMUX_PKG_MAINTAINER="@termux"
# When updating version here, please update termux_setup_cmake.sh as well.
TERMUX_PKG_VERSION=CMAKE_VER
TERMUX_PKG_REVISION=1
TERMUX_PKG_SRCURL=https://www.cmake.org/files/v${TERMUX_PKG_VERSION:0:4}/cmake-${TERMUX_PKG_VERSION}.tar.gz
TERMUX_PKG_SHA256=1c511d09516af493694ed9baf13c55947a36389674d657a2d5e0ccedc6b291d8
TERMUX_PKG_BUILD_DEPENDS="libarchive, libc++, libcurl, libexpat, jsoncpp, libuv, rhash, zlib, libandroid-spawn"
TERMUX_PKG_FORCE_CMAKE=true
TERMUX_PKG_RECOMMENDS="clang, make"
TERMUX_PKG_EXTRA_CONFIGURE_ARGS="
-DSPHINX_MAN=ON
-DCMAKE_MAN_DIR=share/man
-DCMAKE_DOC_DIR=share/doc/cmake
-DCMAKE_USE_SYSTEM_CURL=FALSE
-DCMAKE_USE_SYSTEM_EXPAT=FALSE
-DCMAKE_USE_SYSTEM_FORM=FALSE
-DCMAKE_USE_SYSTEM_JSONCPP=FALSE
-DCMAKE_USE_SYSTEM_LIBARCHIVE=FALSE
-DCMAKE_USE_SYSTEM_LIBRHASH=FALSE
-DCMAKE_USE_SYSTEM_LIBUV=FALSE
-DCMAKE_USE_SYSTEM_ZLIB=FALSE
-DBUILD_CursesDialog=ON"
